...
func NewBotDBWrapper(imgStore storage.ImageStore) (*BoltDBWrapper, error) {
	const perms = 0o600

	boltDB, err := bolt.Open(path.Join(imgStore.RootDir(), "my.db"), perms, &bolt.Options{Timeout: time.Second * 10})
	if err != nil {
		return nil, err
	}
...
func (bdw BoltDBWrapper) GetMultipleRepoMeta(ctx context.Context, filter func(repoMeta RepoMetadata) bool,
	...
	paginator, err := NewBaseRepoPaginator(requestedPage.Limit, requestedPage.Offset, requestedPage.SortBy)
	if err != nil {
		return nil, err
	}
...
func (bdw BoltDBWrapper) AddManifestDependant(manifestDigest string, dependantDigest string) error {
...
func (bdw BoltDBWrapper) SearchTags(ctx context.Context, searchText string, requestedPage PageInput,
	...
	paginator, err := NewBaseImagePaginator(requestedPage.Limit, requestedPage.Offset, requestedPage.SortBy)
	if err != nil {
		return []RepoMetadata{}, map[string]ManifestMetadata{}, err
	}
