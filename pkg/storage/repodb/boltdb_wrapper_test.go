...
func TestBoltDBWrapper(t *testing.T) {
	Convey("BoltDB Wrapper creation", t, func() {
		...
		searchDB, err := repodb.NewBotDBWrapper(mockImageStore)
		So(searchDB, ShouldNotBeNil)
		So(err, ShouldBeNil)

		err = os.Chmod("my.db", 0o200)
		So(err, ShouldBeNil)
	...
	Convey("Test RepoDB Interface implementation", t, func() {
		...
		repoDB, err := repodb.NewBotDBWrapper(mockImageStore)
		...
		Convey("Test AddManifestDependant", func() {
		})
