...
func TestPagination(t *testing.T) {
	Convey("Repo Pagination", t, func() {
		Convey("reset", func() {
			paginator, err := repodb.NewBaseRepoPaginator(1, 0, repodb.AlphabeticAsc)
			So(err, ShouldBeNil)
	...
	Convey("Image Pagination", t, func() {
		Convey("create new paginator errors", func() {
			paginator, err := repodb.NewBaseImagePaginator(-1, 10, repodb.AlphabeticAsc)
			So(paginator, ShouldBeNil)
			So(err, ShouldNotBeNil)

			paginator, err = repodb.NewBaseImagePaginator(2, -1, repodb.AlphabeticAsc)
			So(paginator, ShouldBeNil)
			So(err, ShouldNotBeNil)

			paginator, err = repodb.NewBaseImagePaginator(2, 1, "wrong sorting criteria")
			So(paginator, ShouldBeNil)
			So(err, ShouldNotBeNil)
		})

		Convey("Reset", func() {
			paginator, err := repodb.NewBaseImagePaginator(1, 0, repodb.AlphabeticAsc)
			So(err, ShouldBeNil)
			So(paginator, ShouldNotBeNil)
		...
		Convey("Page", func() {
			Convey("limit < len(tags)", func() {
				paginator, err := repodb.NewBaseImagePaginator(5, 2, repodb.AlphabeticAsc)
				So(err, ShouldBeNil)
				So(paginator, ShouldNotBeNil)
			...
			Convey("limit > len(tags)", func() {
				paginator, err := repodb.NewBaseImagePaginator(3, 0, repodb.AlphabeticAsc)
				So(err, ShouldBeNil)
				So(paginator, ShouldNotBeNil)
