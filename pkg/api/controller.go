	...
	if c.Config.Storage.RootDirectory != "" {
		...
		if len(c.Config.Storage.StorageDriver) == 0 {
			defaultStore = storage.NewImageStore(c.Config.Storage.RootDirectory,
				c.Config.Storage.GC, c.Config.Storage.GCDelay, c.Config.Storage.Dedupe, c.Config.Storage.Commit, c.Config.HTTP.AllowOverwrites, c.Log, c.Metrics)
		} else {
			...
			for route, storageConfig := range subPaths {
				...
				if len(storageConfig.StorageDriver) == 0 {
					subImageStore[route] = storage.NewImageStore(storageConfig.RootDirectory,
						storageConfig.GC, storageConfig.GCDelay, storageConfig.Dedupe, storageConfig.Commit, c.Config.HTTP.AllowOverwrites, c.Log, c.Metrics)
				} else {
