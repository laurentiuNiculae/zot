...
func TestGlobalSearch(t *testing.T) {
	Convey("globalSearch", t, func() {
		...
		Convey("RepoDB SearchRepos error", func() {
			...
			repos, images, layers, err := globalSearch(context.Background(), query, mockSearchDB, &gql_generated.PageInput{},
				log.NewLogger("debug", ""))
		...
		Convey("RepoDB SearchRepo is successful", func() {
			...
			repos, images, layers, err := globalSearch(context.Background(), query, mockSearchDB, &pageInput,
				log.NewLogger("debug", ""))
			So(err, ShouldBeNil)
			So(images, ShouldBeEmpty)
			So(layers, ShouldBeEmpty)
