...
// uploadTestImages is paired with testSetup and is supposed to trigger events when pushing an image
// by pushing just the manifest.
func uploadTestImages(port, baseURL string) error {
...
func TestRepoListWithNewestImage(t *testing.T) {
	...
	Convey("Test repoListWithNewestImage by tag with HTTP", t, func() {
		...
		err = uploadTestImages(port, GetBaseURL(port))
		So(err, ShouldBeNil)
